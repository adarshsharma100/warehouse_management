...
async function main() {
  console.log("--- STARTING FAST SHOPIFY SOURCENAME BACKFILL ---");
  ...
  while (url) {
    ...
    const matchingOrders = orders.filter(o => recordMap.has(String(o.id)));
    console.log(`Fetched ${orders.length} orders from Shopify (${matchingOrders.length} matches). Updating database...`);

    let pageUpdated = 0;
    const chunkSize = 50;
    for (let i = 0; i < matchingOrders.length; i += chunkSize) {
      const chunk = matchingOrders.slice(i, i + chunkSize);
      await Promise.all(
        chunk.map(order => {
